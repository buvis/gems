...
- **pidash**: `pidash hooks install` now fails with a clear error and leaves `settings.json` untouched when the existing `hooks` key is not a JSON object (e.g. accidentally an array or string). Previously it silently overwrote the value with `{}` and rewrote the file, which could discard the user's hook config.
