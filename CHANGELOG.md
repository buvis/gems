...
- **dot**: the TUI now shows a changed git-secret file in its unstaged pane on first render, instead of only after a prior CLI `dot status` call has hidden it. A failure while hiding secrets is now surfaced as an on-screen error instead of silently leaving the displayed status stale (#92).
