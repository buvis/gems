...
- **fctracker**: an overdrawn account, a zero-amount withdrawal, a malformed amount cell, or a non-newest-first CSV now returns a friendly error naming the account, instead of crashing with a raw traceback (`balance`) or a blank/garbled cause (`transactions`).
