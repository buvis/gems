...
class TestAddUrl:
    ...
    @patch("readerctl.adapters.reader_api.reader_api.requests")
    @patch("readerctl.adapters.reader_api.reader_api.console")
    def test_add_url_rate_limited_invalid_retry_after(self, mock_console, mock_requests, mock_time):
        rate_limited = MagicMock()
        rate_limited.status_code = 429
    ...
    @patch("readerctl.adapters.reader_api.reader_api.requests")
    def test_add_url_200_existing(self, mock_requests):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"url": "https://reader.example.com/doc/existing"}
        mock_requests.post.return_value = mock_response
