...
class TestReaderctlCommands:
    ...
    def test_login_success(self, mock_cmd_cls: MagicMock, runner) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=True, output="Logged in")
        result = runner.invoke(cli, ["login"])
        assert result.exit_code == 0

    @patch("readerctl.commands.login.login.CommandLogin")
    def test_login_failure(self, mock_cmd_cls: MagicMock, runner) -> None:
    ...
    @patch("readerctl.commands.login.login.CommandLogin")
    def test_add_url_success(self, mock_login_cls: MagicMock, mock_add_cls: MagicMock, runner) -> None:
    ...
    @patch("readerctl.commands.add.add.CommandAdd")
    @patch("readerctl.commands.login.login.CommandLogin")
    def test_add_no_token_panics(self, mock_login_cls: MagicMock, mock_add_cls: MagicMock, runner) -> None:
        mock_login_cls.return_value.execute.return_value = CommandResult(
            success=True,
            output="Logged in",
            metadata={},  # no token
