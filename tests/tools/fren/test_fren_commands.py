...
class TestFrenCommands:
    ...
    @patch("fren.commands.slug.slug.CommandSlug")
    def test_slug_with_warnings(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        target = tmp_path / "a.txt"
        target.write_text("x")
    ...
    @patch("fren.commands.slug.slug.CommandSlug")
    def test_slug_success_no_output(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        target = tmp_path / "a.txt"
        target.write_text("x")
    ...
    @patch("fren.commands.slug.slug.CommandSlug")
    def test_slug_failure_no_error(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        target = tmp_path / "a.txt"
        target.write_text("x")
    ...
    @patch("fren.commands.directorize.directorize.CommandDirectorize")
    def test_directorize_failure(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
    ...
    @patch("fren.commands.directorize.directorize.CommandDirectorize")
    def test_directorize_with_warnings(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(
    ...
    @patch("fren.commands.directorize.directorize.CommandDirectorize")
    def test_directorize_success_no_output(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=True)
        runner = CliRunner()
        result = runner.invoke(cli, ["directorize", str(tmp_path)])
        assert result.exit_code == 0
    ...
    @patch("fren.commands.flatten.flatten.CommandFlatten")
    def test_flatten_failure(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        src = tmp_path / "src"
        src.mkdir()
        dst = tmp_path / "dst"
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=False, error="bad")
    ...
    @patch("fren.commands.flatten.flatten.CommandFlatten")
    def test_flatten_with_warnings(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        src = tmp_path / "src"
        src.mkdir()
        dst = tmp_path / "dst"
        mock_cmd_cls.return_value.execute.return_value = CommandResult(
    ...
    @patch("fren.commands.flatten.flatten.CommandFlatten")
    def test_flatten_success_no_output(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        src = tmp_path / "src"
        src.mkdir()
        dst = tmp_path / "dst"
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=True)
    ...
    @patch("fren.commands.normalize.normalize.CommandNormalize")
    def test_normalize_failure(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=False, error="bad")
        runner = CliRunner()
        result = runner.invoke(cli, ["normalize", str(tmp_path)])
        assert result.exit_code == 0
    ...
    @patch("fren.commands.normalize.normalize.CommandNormalize")
    def test_normalize_with_warnings(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(
    ...
    @patch("fren.commands.normalize.normalize.CommandNormalize")
    def test_normalize_success_no_output(self, mock_cmd_cls: MagicMock, tmp_path) -> None:
        mock_cmd_cls.return_value.execute.return_value = CommandResult(success=True)
        runner = CliRunner()
        result = runner.invoke(cli, ["normalize", str(tmp_path)])
        assert result.exit_code == 0
