...
@pytest.fixture
def rich_console(console_stream: StringIO) -> Console:
...
class TestCapturingRichHandler:
    def test_init_stores_console(self, rich_console: Console) -> None:
        handler = CapturingRichHandler(rich_console)
        assert handler.console is rich_console

    def test_emit_calls_format_render_and_print_in_order(
        self, monkeypatch: pytest.MonkeyPatch, rich_console: Console, log_record: LogRecord
    ) -> None:
        handler = CapturingRichHandler(rich_console)
        calls: list[str] = []
        ...
        monkeypatch.setattr(rich_console, "print", fake_print, raising=True)
    ...
    def test_emit_outputs_to_console(
        self, console_stream: StringIO, rich_console: Console, log_record: LogRecord
    ) -> None:
        handler = CapturingRichHandler(rich_console, show_time=False, show_path=False)
        handler.emit(log_record)
        assert "fixture message" in console_stream.getvalue()

    def test_works_with_logging(self, console_stream: StringIO, rich_console: Console) -> None:
        handler = CapturingRichHandler(rich_console, show_time=False, show_path=False)
